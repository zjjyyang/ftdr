...
mzMLWriter::writeDocument(void)	  
{
	...
	if (!(isThermo||isWaters)) {
		printf("unknown manufacturer: %s\n",toString(instrumentInterface_->instrumentInfo_.manufacturer_).c_str());
		return;
	}
	...
	if (instrumentInterface_->instrumentInfo_.analyzerList_.size() == 0) {
		...
		printf("can't deal with more than one analyzer yet.\n");
		return;
	}
	...
	if (programName_ == "ReAdW") {
		...
		printf("unknown converter name %s\n",programName_.c_str());
		return;
	}
	...
	for (long curScanNum=1; curScanNum <= instrumentInterface_->totalNumScans_; curScanNum++) {
		...
		//这里是关键，决定从这个函数动手，处理图谱
		if (verbose_)
		{
			if ((curScanNum % 100) == 0) 
			{
				printf("%d/%d\r",curScanNum,instrumentInterface_->totalNumScans_);
				//cout.flush();
			}
		...
		if (scanType == SCAN_UNDEF) {
			...
			printf("please contact developer.\n");
			return;
		}
		else {
	...
	if( !(sha1_.HashGZFile(outputFileName_.c_str()))) {
		...
		return;// Cannot open file for sha1
	}
	sha1_.Final();
	sha1_.ReportHash(sha1Report_, SHA1::REPORT_HEX);
	printf("--done (mzML sha1): %s\n" ,sha1Report_);
