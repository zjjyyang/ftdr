...
#include "gzstream.h"// Needed for handling potentially gzipped files
...
bool SHA1::HashFile(const char *szFileName)
{
	...
	{
		size_t nread = fread(uData, 1, MAX_FILE_READ_BUFFER, fIn);
...
// Hash in file contents, if file is gzipped we'll hash the unzipped contents
bool SHA1::HashGZFile(const char *szFileName)
{
	unsigned char uData[MAX_FILE_READ_BUFFER];
	igzstream fIn(szFileName); // open a potentially gzipped file

	if (!fIn.is_open()) return(false);

	while (true)
	{
		fIn.read((char *)uData, MAX_FILE_READ_BUFFER);
