...
void force_unCygwinify(char *filename) {
  ...
  for (cp=filename;*cp;cp++) {
     if ('\\'==*cp) { // we want consistent path seperators
        *cp = '/';
     }
  }
}
...
int do_gzip(std::string &fname) {
	...
	{ // scope the constructor/destructor so file handle isn't locked after we use it here
		pwiz::util::random_access_compressed_ifstream test(fname.c_str());
...
void replace_path_with_webserver_tmp(char *path,int pathmaxlen) { // write this in tmpdir if we have one
	const char *szWebserverTmp=getWebserverTmpPath();
	if (szWebserverTmp) {
#ifdef WINDOWS_CYGWIN  // watch out for mix of /cygdrive/ hoohah and regular win32 paths
		std::string cygpath(path);
		std::string cygtmp(szWebserverTmp);
		if (isCygwinishPath(path)) { // are we dealing with cygwin-speak?
		  cygwinify(cygtmp);
		}
		if (strncasecmp(cygpath.c_str(),cygtmp.c_str(),strlen(cygtmp.c_str()))) {
			replace_path(cygpath,cygtmp.c_str());
		}
		strncpy(path,cygpath.c_str(),pathmaxlen);
#else
...
static char localhtml[1024]={0};
const char *getLocalHTML() {
#if (defined(__CYGWIN__)||defined(WINDOWS_NATIVE))
	if (!localhtml[0]) { // first access?
		std::string html(getTrueWebserverRoot());
		if (html[html.length()-1]!='/') {
			html+="/";
		}
		html+= (getHTMLDir()+(*getHTMLDir() == '/'));
		strncpy(localhtml,html.c_str(),sizeof(localhtml));
   }
   return localhtml;
#else
	return DEFAULT_LOCAL_HTML;
#endif
}

static char cgibin[1024]={0};
const char *getCGIFullBin() {
#if defined __CYGWIN__ || defined WINDOWS_NATIVE
	if (!cgibin[0]) { // first access?
		std::string cgi(getTrueWebserverRoot());
		if (cgi[cgi.length()-1]!='/') {
			cgi+="/";
		}
		cgi += "../";
		cgi += (DEFAULT_CGI_BIN+(*DEFAULT_CGI_BIN == '/'));
		strncpy(cgibin,cgi.c_str(),sizeof(cgibin));
	}
   return cgibin;
#else
   return DEFAULT_CGI_FULL_BIN;
#endif
}


#ifdef WINDOWS_CYGWIN
void cygwinify(char *fname, int maxlen) {
	std::string s(fname);
	cygwinify(s);
	strncpy(fname,s.c_str(),maxlen);
}

void cygwinify(std::string &fname) {
	char szCommand[1024];
	FILE *fp;
	sprintf(szCommand, "cygpath '%s'", fname.c_str());
	if((fp = popen(szCommand, "r")) == NULL)
	{
		printf("cygpath error, exiting\n");
		exit(1);
	}
	fgets(szCommand, sizeof(szCommand), fp);
	pclose(fp);
...
// returns:
//   malloc'd path (caller must free) of file that can be opened,
...
char *resolve_root(const char *path) {
  ...
  if (stat(path,&statbuf)) { // didn't find it
     char *slash;
     char *buf = prepend_helper(path,getTrueWebserverRoot()); // is it in the wwwroot area?
     if (!stat(buf,&statbuf)) {
        // that worked
        return buf;
     }
	 ...
	 free(buf);
     buf = prepend_helper(path,"c:/cygwin/");
     if (!stat(buf,&statbuf)) {
        // that worked
        return buf;
     }
#endif
     // try cwd
     slash = findRightmostPathSeperator(buf);
     if (slash) {
...
int safe_rename(const char *from, const char *to) {
  ...
  fromto += to;
  fromto += quot;
#ifdef EC2  // building for Amazon Web Services EC2
  // when crossing the EC2/S3 boundary, mv is unreliable (probably actually an S3FS issue)
  // so use cp instead as needed
  std::string frompath(from);
  makeFullPath(frompath);
  std::string topath(to);
  makeFullPath(topath);
  // OK, are we going to or from the S3 filesystem?
  if (strstr(frompath.c_str(),getWebserverRoot())||
      strstr(topath.c_str(),getWebserverRoot())) {
     std::string cmd = "cp -f "; // cp seems safest for dealing with S3 xfer on AWS EC2 
	 cmd += fromto;
	 result = tpplib_system(cmd.c_str());
	 if (!result) {
		 result = verified_unlink(from);
	 }
  } else
#endif
  {  
...
std::string uncompress_to_tmpfile(const char *fname,int maxchar) {
	...
	if (!fname || !*fname) {
		...
		{ // scope the constructor/destructor so file handle isn't locked after we use it here
			pwiz::util::random_access_compressed_ifstream test(fname);
